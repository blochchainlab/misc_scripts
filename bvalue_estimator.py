...
def main():
    ...
    for i in range(len(unique_bvalues)-1):
        plt.figure()
        tit = 'MD^-1, first {} shells (bmax = {})'.format(i+2, bvals[shells==i+1].max())
        ...
        # vv2 = tmp.shape[0]
        print('quantile threshold removed {} voxels'.format(np.logical_and(tmp >= minval, tmp <= maxval).sum()))
        ...
        plt.axvline(onequart, color='pink', label='25% ({:.0f})'.format(onequart))
        plt.axvline(twoquart, color='yellow', label='50% ({:.0f})'.format(twoquart))
        plt.axvline(threequart, color='green', label='75% ({:.0f})'.format(threequart))
        plt.title(tit)
        plt.legend(loc=1)
